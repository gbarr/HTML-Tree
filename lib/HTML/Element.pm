...
## RT #42209 why does this default to the attribute name and not remain unset or the empty string?
    ...
    Scalar::Util::weaken($self->{'_parent'}) if $self->{'_parent'};
    if ( $tag eq 'html' ) {
        $self->{'_pos'} = undef;
    }
    return $self;
...
sub parent {
    ...
    if (@_) {    # set
        ...
        Scalar::Util::weaken($self->{'_parent'} = $_[0]);
    }
    else {
        $self->{'_parent'};                                       # get
    }
...
sub push_content {
    ...
    for (@_) {
        if ( ref($_) eq 'ARRAY' ) {
            ...
            Scalar::Util::weaken($content->[-1]->{'_parent'} = $self);
        }
        elsif ( ref($_) ) {    # insert an element
            $_->detach if $_->{'_parent'};
            Scalar::Util::weaken($_->{'_parent'} = $self);
            push( @$content, $_ );
        }
...
sub unshift_content {
    ...
    for ( reverse @_ ) {    # so they get added in the order specified
        if ( ref($_) eq 'ARRAY' ) {
            ...
            Scalar::Util::weaken($content->[0]->{'_parent'} = $self);
        }
        elsif ( ref $_ ) {    # insert an element
            $_->detach if $_->{'_parent'};
            Scalar::Util::weaken($_->{'_parent'} = $self);
    ...
    if ( @_ > 2 ) {    # self, offset, length, ...
        foreach my $n (@to_add) {
            if ( ref($n) eq 'ARRAY' ) {
                ...
                Scalar::Util::weaken($n->{'_parent'} = $self);
            }
            elsif ( ref($n) ) {
                $n->detach;
                Scalar::Util::weaken($n->{'_parent'} = $self);
...
sub replace_with {
    ...
    for (@replacers) {
        if ( !ref $_ ) {
            ...
            Scalar::Util::weaken($_->{'_parent'} = $parent);
        }
        else {
            $_->detach;
            Scalar::Util::weaken($_->{'_parent'} = $parent);
...
sub replace_with_content {
    ...
    for ( splice @$content_r ) { 
        Scalar::Util::weaken($_->{'_parent'} = $parent) if ref $_;
    }
...
sub clone {
    ...
    if ( $it->{'_content'} and @{ $it->{'_content'} } ) {
        ...
        for ( @{ $new->{'_content'} } ) {
            Scalar::Util::weaken($_->{'_parent'} = $new) if ref $_;
        }
    }
        ...
        if ( $class eq __PACKAGE__ ) {    # Special-case it, for speed:
            ...
            if (@children) {
                ...
                foreach my $c (@children) {
                    Scalar::Util::weaken($c->{'_parent'} = $node)
                        if ref $c;
                }
            }
        }
            ...
            $node->push_content(
                map { Scalar::Util::weaken($_->{'_parent'} = $node) if ref $_; $_ } @children )
                if @children;
        }
